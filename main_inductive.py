...
from augmae.datasets.data_util import load_inductive_dataset
from augmae.models import build_model
from augmae.evaluation import linear_probing_for_inductive_node_classiifcation, LogisticRegression
...
def pretrain(model, dataloaders, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob,args, mask_module, optimizer_mask, logger=None):
    ...
    for epoch in epoch_iter:
        ...
        for subgraph in train_loader:
            ...
            mask_prob1 = mask_module.forward(subgraph, subgraph.ndata["feat"],args)
            loss, loss_mask, loss_dict = model(subgraph, subgraph.ndata["feat"], epoch, args, mask_prob1,0.)
...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        if not load_model:
            model = pretrain(model, (train_dataloader, valid_dataloader, test_dataloader, eval_train_dataloader), optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob, args,mask_module,optimizer_mask,logger)
        model = model.cpu()
